...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-6 mb-10">
            ...
            <div>
                <h1 class="text-3xl font-black text-slate-900 tracking-tight">Edit Student Profile</h1>
                <p class="text-slate-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">Modify existing membership data</p>
            </div>
        </div>
            ...
            <div class="p-10">
                <form action="{{ route('students.update', $student) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                        ...
                        <div>
                            ...
                            <input type="text" id="student_id" name="student_id" value="{{ old('student_id', $student->student_id) }}"
                                   class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-bold text-slate-900 @error('student_id') ring-2 ring-rose-500 @enderror"
                                   required>
                            @error('student_id')
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $student->name) }}"
                                   class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-bold text-slate-900 @error('name') ring-2 ring-rose-500 @enderror"
                                   required>
                            @error('name')
                                <p class="text-rose-600 text-[10px] font-bold mt-2 px-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="email" id="email" name="email" value="{{ old('email', $student->email) }}"
                                   class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-bold text-slate-900 @error('email') ring-2 ring-rose-500 @enderror"
                                   required>
                            @error('email')
                                <p class="text-rose-600 text-[10px] font-bold mt-2 px-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" id="contact_number" name="contact_number" value="{{ old('contact_number', $student->contact_number) }}"
                                   class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-bold text-slate-900 @error('contact_number') ring-2 ring-rose-500 @enderror">
                        ...
                        <div>
                            ...
                            <input type="text" id="course" name="course" value="{{ old('course', $student->course) }}"
                                   class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-bold text-slate-900 @error('course') ring-2 ring-rose-500 @enderror"
                                   required>
                            @error('course')
                                <p class="text-rose-600 text-[10px] font-bold mt-2 px-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select id="year_level" name="year_level"
                                ...
                                <option value="1" @selected(old('year_level', $student->year_level) == 1)>1st Year (Freshman)</option>
                                <option value="2" @selected(old('year_level', $student->year_level) == 2)>2nd Year (Sophomore)</option>
                                <option value="3" @selected(old('year_level', $student->year_level) == 3)>3rd Year (Junior)</option>
                                <option value="4" @selected(old('year_level', $student->year_level) == 4)>4th Year (Senior)</option>
                            </select>
                            @error('year_level')
                                <p class="text-rose-600 text-[10px] font-bold mt-2 px-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="flex flex-col md:flex-row gap-4 pt-6 border-t border-slate-50">
                        <button type="submit" 
                                class="flex-1 bg-slate-900 hover:bg-amber-600 text-white font-black uppercase tracking-[0.2em] text-xs py-5 rounded-2xl transition-all duration-300 shadow-xl shadow-slate-200 hover:shadow-amber-200 hover:-translate-y-1">
                            Update Registration
                        </button>
                        <a href="{{ route('students.index') }}" 
